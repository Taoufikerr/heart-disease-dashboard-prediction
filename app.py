...
if page == "📊 Dashboard":
    ...
    cp_filter = st.sidebar.selectbox("Chest Pain Type (cp)", ["All"] + list(cp_options.keys()))
    ...
    # Inline Prediction
    st.subheader("🔍 Quick Prediction (Inline)")
...
if page == "🤖 Full Prediction Interface":
    ...
    sex_label = col2.selectbox("Sex", ["Male", "Female"])
    sex = 1 if sex_label == "Male" else 0
    cp_label = col1.selectbox("Chest Pain Type", list(cp_options.keys()))
    cp = cp_options[cp_label]
    trestbps = col2.slider("Resting BP", 80, 200, 120)
    chol = col1.slider("Cholesterol", 100, 600, 250)
    fbs_label = col2.selectbox("Fasting Blood Sugar", list(fbs_options.keys()))
    fbs = fbs_options[fbs_label]
    restecg_label = col1.selectbox("Resting ECG", list(restecg_options.keys()))
    restecg = restecg_options[restecg_label]
    thalach = col2.slider("Max Heart Rate", 60, 220, 150)
    exang_label = col1.selectbox("Exercise Induced Angina", list(exang_options.keys()))
    exang = exang_options[exang_label]
    oldpeak = col2.slider("ST Depression", 0.0, 6.0, 1.0)
    slope_label = col1.selectbox("Slope", list(slope_options.keys()))
    slope = slope_options[slope_label]
    ca = col2.slider("CA", 0, 3, 0)
    thal_label = col1.selectbox("Thal", list(thal_options.keys()))
    thal = thal_options[thal_label]
