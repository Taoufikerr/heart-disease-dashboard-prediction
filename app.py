...
if len(date_range) == 2:
    ...
    st.plotly_chart(px.histogram(filtered_df, x="ca", color=filtered_df["target"].map({0: "No Heart Disease", 1: "Heart Disease"}), barmode="group", title="Major Vessels by Heart Disease"))
    st.plotly_chart(px.box(filtered_df, x="target", y="chol", color=filtered_df["target"].map({0: "No Heart Disease", 1: "Heart Disease"}), title="Cholesterol by Heart Disease"))
    st.plotly_chart(px.bar(filtered_df, x="restecg", color=filtered_df["target"].map({0: "No Heart Disease", 1: "Heart Disease"}), barmode="group", title="Resting ECG vs Target"))
...
if page == "🤖 Full Prediction Interface":
    ...
    if prediction == 1:
        ...
        st.success("These values indicate low risk for heart disease."if page == "🤖 Full Prediction Interface":
    st.title("Full Heart Disease Prediction Interface")
